...
function stylish($ast, $depth = 0): string
{
    ...
    $result = array_map(function ($item) use ($depth, $statuses, $indent) {
        if ($item['status'] === 'deleted') {
            $value = is_array($item['value']) ? stylish($item['value'], $depth + 1) : renderValue($item['value']);
        ...
        if ($item['status'] === 'changed') {
            $oldValue = is_array($item['oldValue']) ? stylish($item['oldValue'], $depth + 1) : renderValue($item['oldValue']);
            $newValue = is_array($item['newValue']) ? stylish($item['newValue'], $depth + 1) : renderValue($item['newValue']);
            return implode("\n", [
